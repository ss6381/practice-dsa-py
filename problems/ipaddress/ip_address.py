...
def access(rules: List[Tuple[str, str]], ip_address: str) -> str:
    priority: List[str] = []
    for allow_deny, rule_address in rules:
        priority.append((allow_deny, expand_rule(rule_address)))
    for allow_deny, r in priority:
        if in_range(r, ip_address):
            return allow_deny
    return "DENY"


def expand_rule(rule_address: str) -> List[str]:
    """
    ...
    def bit_bound(
        ip_split: List[str], section_step: int, bit_index: int, val: int
    ) -> str:
        ip_split_copy: List[str] = ip_split.copy()
        address = "{0:08b}".format(int(ip_split_copy[section_step]))
        shifted_address = address[0:bit_index] + "".join(
            [str(val) for i in range(len(address) - bit_index)]
        )
...
def in_range(r: str, ip_address: str) -> bool:
    ip_split = ip_address.split(".")
    lower_split = r[0].split(".")
    upper_split = r[1].split(".")
    for i in range(len(ip_split)):
        if int(ip_split[i]) < int(lower_split[i]) or int(ip_split[i]) > int(
            upper_split[i]
        ):
            return False
    return True
